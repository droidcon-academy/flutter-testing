...
void main() {
  group('Navigation States Golden Tests (Implementation-Aware)', () {
    setUpAll(() async {
      await loadAppFonts();
    });

    testGoldens('Navigation - ResponsiveLayoutBuilder (mobile/tablet/desktop)',
        (tester) async {
      int selectedIndex = 0;
      ...
      await screenMatchesGolden(tester, 'navigation_states_responsive_mobile');
      ...
      expect(find.byType(NavBar), findsOneWidget);
      expect(find.byType(NavRail), findsNothing);
      await screenMatchesGolden(tester, 'navigation_states_responsive_tablet');
      ...
      await screenMatchesGolden(tester, 'navigation_states_responsive_desktop');

      addTearDown(tester.view.resetPhysicalSize);
    });

    testGoldens('NavBar - Isolated visual regression', (tester) async {
      final widget = MaterialApp(
        theme: AppTheme.lightTheme,
        home: ProviderScope(
          child: Container(
      ...
      await multiScreenGolden(
        tester,
        'navigation_states_nav_bar_isolated',
        devices: [const Device(size: Size(375, 80), name: 'nav_bar_only')],
      );
    });
    ...
    testGoldens('NavRail - Isolated visual regression', (tester) async {
      ...
      await multiScreenGolden(
        tester,
        'navigation_states_nav_rail_isolated',
